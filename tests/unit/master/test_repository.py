...
class RepositoryMessageConsumerTest(TestCase):
    ...
    def tearDown(self):
        repository.RepositoryMessageConsumer._stop_consuming_messages = False
    ...
    async def test_add_builds(self):
        ...
        message_consumer = repository.RepositoryMessageConsumer()
    ...
    @patch.object(repository.RepositoryMessageConsumer, '_add_builds',
                  AsyncMagicMock())
    ...
    async def test_wait_revisions(self):
        ...
        async def fm(cancel_on_timeout=False):
            repository.RepositoryMessageConsumer.\
                _stop_consuming_messages = True

        consumer.fetch_message = fm
        message_consumer = repository.RepositoryMessageConsumer()
        await message_consumer.wait_revisions()
        self.assertTrue(message_consumer._add_builds.called)
    ...
    async def test_wait_revisions_timeout(self):
        ...
        message_consumer = repository.RepositoryMessageConsumer()
        await message_consumer.wait_revisions()
        self.assertFalse(message_consumer._add_builds.called)

    @patch.object(repository.repo_notifications, 'consume',
                  AsyncMagicMock(spec=repository.repo_notifications.consume))
    @patch.object(repository.RepositoryMessageConsumer, '_add_requested_build',
                  AsyncMagicMock(
                      spec=repository.RepositoryMessageConsumer.
                      _add_requested_build))
    @async_test
    async def test_wait_build_requests(self):
        ...
        consumer.fetch_message = fm
        message_consumer = repository.RepositoryMessageConsumer()
        await message_consumer.wait_build_requests()
        self.assertTrue(
            repository.RepositoryMessageConsumer._add_requested_build.called)
    ...
    @patch.object(repository.RepositoryMessageConsumer, '_get_repo_from_msg',
                  AsyncMagicMock(
                      spec=repository.RepositoryMessageConsumer.
                      _get_repo_from_msg,
                      return_value=None))
    ...
    @patch.object(repository.RepositoryMessageConsumer, '_get_repo_from_msg',
                  AsyncMagicMock(spec=repository.RepositoryMessageConsumer.
                                 _get_repo_from_msg,
    ...
    async def test_remove_repo_exception(self):
        repo = repository.RepositoryMessageConsumer._get_repo_from_msg.\
            return_value
    ...
    async def test_update_repo(self):
        repo = repository.RepositoryMessageConsumer.\
    ...
    @patch.object(repository.repo_notifications, 'consume',
                  AsyncMagicMock(spec=repository.repo_notifications.consume))
    @patch.object(repository.RepositoryMessageConsumer, '_remove_repo',
                  AsyncMagicMock(
                      spec=repository.RepositoryMessageConsumer._remove_repo))
    @async_test
    async def test_wait_removal_request(self):
        ...
        consumer.fetch_message = fm
        message_consumer = repository.RepositoryMessageConsumer()
        await message_consumer.wait_removal_request()
        kw = repository.repo_notifications.consume.call_args[1]
        self.assertTrue(message_consumer._remove_repo.called)
    ...
    async def test_wait_removal_request_timeout(self):
        ...
        message_consumer = repository.RepositoryMessageConsumer()
        await message_consumer.wait_removal_request()
        self.assertFalse(message_consumer._remove_repo.called)

    @patch.object(repository.repo_notifications, 'consume',
                  AsyncMagicMock(spec=repository.repo_notifications.consume))
    @patch.object(repository.RepositoryMessageConsumer, '_update_repo',
                  AsyncMagicMock(
                      spec=repository.RepositoryMessageConsumer._update_repo))
    @async_test
    async def test_wait_upate_request(self):
        ...
        consumer.fetch_message = fm
        message_consumer = repository.RepositoryMessageConsumer()
        await message_consumer.wait_update_request()

    def test_run(self):
        message_consumer = repository.RepositoryMessageConsumer()
        message_consumer.wait_revisions = AsyncMagicMock(
            spec=message_consumer.wait_revisions)
        message_consumer.wait_build_requests = AsyncMagicMock(
            spec=message_consumer.wait_build_requests)
        message_consumer.wait_removal_request = AsyncMagicMock(
            spec=message_consumer.wait_removal_request)
        message_consumer.wait_update_request = AsyncMagicMock(
            spec=message_consumer.wait_update_request)

        message_consumer.run()
        self.assertTrue(all([message_consumer.wait_revisions.called,
                             message_consumer.wait_build_requests.called,
                             message_consumer.wait_revisions.called,
                             message_consumer.wait_update_request.called]))
